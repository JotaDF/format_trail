...
Version 3.5.0.2
  1. Fix CONTRIB-7270 - Popovers not working as expected in Trail reset options section of settings - Boost theme.
  2. Foundation theme support.
  3. Fix: Boost theme shadebox adjustment.
  4. Fix: Convert displayed image to original mime type instead of only png patch when section zero in the trail.
...
Version 3.4.0.3
  1. Fix double entry for a course in the format_trail_summary when restoring.
  2. Fix logic of showing trail boxes depending on if it is visible against the grey out setting and course 'Hidden sections' option.
...
Version 3.4.0.1
  ...
  4. Trail with essential fixed nav - nav bar overlaps the trail section when opened - #64.
  5. Add the ability to have section 0 on its own page when out of the trail and the 'Course layout' is 'One section per page'.  Gratefully funded by Coventry University UK.
...
Version 3.2.1.3
  ...
  4. Fix "trailcontainer alignment" - #54.
...
Version 3.2.1.2
 ...
 16. Fix 'Text not correctly escaped in trail popovers' - #52.
...
Version 3.2.1.1
  ...
  3. Fix 'Managers assigned at Course Category levels cannot create a course with trail format selected' - #42.
  4. Fix position of shadebox with Essential / Boost theme - #45.
  5. Fix missing closing 'div'.
  6. Fix position of section title when inside the trail box and section title tooltip width.
...
Version 3.2.1.0
  ...
  2. Remove old 'format_trail_delete_course' function and replace with '\core\event\course_content_deleted' event.
  3. Cope with the Boost theme in relation to the shadebox.
  4. Cope with the Boost theme in relation to the trail area.
...
Version 3.1.1.0
  ...
  3. Fix truncate long section names in the trail - #33.
...
Version 3.0.1.1
  ...
  2. Added the ability to grey out and unlink trail icons for unavailable sections - #30 - thanks to Justin Hunt.
  3. Travis CI fixes.
...
Version 3.0.0.4
  ...
  2. Section 0 not shown on its own when in the Trail and 'Show one section per page' - #24.
  3. TravisCI - tidy up.
...
Version 3.0.0.2
  ...
  2. Fix #22 - Trail sometimes loads with blocks misaligned.
...
Version 2.9.1.4.
  ...
  2. Delete format_trail_icon record when a section is deleted.
...
Version 2.9.1.2.
  ...
  2. Attempt self repair if trail image not in the files table.
...
3rd April 2015 Version 2.8.2.2
  1. Fixed 'importing of trail icons.' - #11 - patch gratefully received from Tim Lock.
...
5th November 2014 Version 2.7.1.4
  ...
  4. Fixed section 0 editing drop down in Bootstrap v3 themes: https://github.com/gjb2048/moodle-format_trail/issues/9.

18th August 2014 Version 2.7.1.3
  1. Fixed issue whereby the trail shade box is underneath certain theme components:
...
Change by G J Barnard
  1. Fixed CONTRIB-5001 - Cannot use the same image in the section summary as well as the trail.
...
Change by G J Barnard
  1. Fix to RTL CSS thanks to Nadav Kavalerchik - https://github.com/gjb2048/moodle-courseformat_trail/pull/3 being:
  ...
  9. Slightly shrunk trail icon outside to make better use of available space.
 10. Changed current section colour in the trail to use a box-shadow instead of a background to avoid confusion with
...
Change by G J Barnard
  ...
  2. Fixed issue reported by Graham Woodsford whereby teachers could not create Collapsed Topics courses.  The code in the area
     concerned is identical to the Trail format and hence the same issue affects it too.  This is because the validation method
     ...
     'format/trail:changeimagecontainersize', 'format/trail:changeimageresizemethod' and 'format/trail:changeimagecontainerstyle'
     ...
     Ok, this now leads on to a dilemma.  Currently the course creator role does not have the Trail capabilities listed above.  If
     they were added to 'access.php' then the role would have them (existing Trail admins would have to add manually).  Then the
...
Change by G J Barnard
  ...
  2. Fix 'reset_trail_setting' in 'lib.php' not resetting a course if it's only on default options.
...
been an issue as there should be no records in the 'format_trail_icon' table for course id '1', but
...
Change by G J Barnard
  1. Fix incorrect detection of non-existent entry in format_trail_icon table.  Thanks to
...
Change by G J Barnard
  ...
  5. Fixed an issue when deleting a course that was not in the trail format where
     the function 'format_trail_delete_course' in 'lib.php' was being called even
...
Change by G J Barnard
  ...
  3. Fixed section 0 content displaying when it's in the trail, you first load a page and click on another
...
Change by G J Barnard
  ...
  2. Removed old 'trailkeys.js' from 'javascript' folder.
...
Change by G J Barnard
  1. Fixed CONTRIB-4216 - Error importing quizzes with trail course format.
...
Change by G J Barnard
  1. Fixed issue where the trail did not function in 'One section per page mode' on the course settings.
